...
class BikeCustomizeScreen extends StatefulWidget {
  const BikeCustomizeScreen({super.key, required this.bike});

  final FirestoreBike bike;

  @override
  State<BikeCustomizeScreen> createState() => _BikeCustomizeScreenState();
}

class _BikeCustomizeScreenState extends State<BikeCustomizeScreen> {
  ...
  static const String _masterModelPath = 'assets/models/kids_mtb_master.glb';
  ...
  static const Duration _customizeShowroomCycleDuration = Duration(
    milliseconds: 3000,
  );
  static const Duration _customizeShowroomResumeDelay = Duration(seconds: 2);

  String _selectedSkinId = 'standard';
  _CustomizeCategory _selectedCategory = _CustomizeCategory.skins;
  ...
  final Interactive3dController _frameController = Interactive3dController();
  ...
  static const List<_FrameVariantOption> _frameVariants = <_FrameVariantOption>[
    _FrameVariantOption(
      id: 'frame_1',
      title: 'frameTrail',
      entityName: 'Frame 1',
  ...
  static const Map<String, String> _monthlyRewardFrameMap = <String, String>{
  ...
  static const Map<String, String> _monthlyRewardSkinMap = <String, String>{
  ...
  void initState() {
    ...
    final initialFrameId =
        _frameVariants.any((frame) => frame.id == savedFrameId)
        ? savedFrameId
        : 'frame_1';
  ...
  Future<void> _refreshEntitlementUser() async {
    ...
    try {
      final user = await FirestoreSkinEntitlementService.instance.getUser(
        ownerId,
      );
  ...
  void _startEntitlementWatch() {
    ...
    _entitlementSubscription = FirestoreSkinEntitlementService.instance
        .watchUser(ownerId)
        .listen(
          (user) {
            if (!mounted) return;
  ...
  void _startFrameEntitlementWatch() {
    ...
    _frameEntitlementSubscription = FirestoreSkinEntitlementService.instance
        ...
        .listen(
          (ownedFrameIds) {
            ...
            setState(() {
              _ownedFrameIds = <String>{'frame_1', ...ownedFrameIds};
            });
          },
          onError: (Object error, StackTrace stackTrace) {
        ...
        .listen(
          (snapshot) {
            ...
            for (final document in snapshot.docs) {
              ...
              final rewardId = (data['rewardId']?.toString() ?? document.id)
                  .trim();
              ...
              if (type == 'frame') {
                final mappedFrameId = _monthlyRewardFrameMap[rewardId];

                if (mappedFrameId != null && mappedFrameId.isNotEmpty) {
                  rewardFrameIds.add(mappedFrameId);
                }
              } else if (type == 'skin') {
                final mappedSkinId = _monthlyRewardSkinMap[rewardId];

                if (mappedSkinId != null && mappedSkinId.isNotEmpty) {
          ...
          onError: (Object error, StackTrace stackTrace) {
            debugPrint('BIKE CUSTOMIZE MONTHLY REWARD WATCH ERROR: $error');
            debugPrint('$stackTrace');
          },
        );
  }
  ...
  bool _isSkinUsable(_SkinOption skin) {
    ...
    switch (skin.access) {
      ...
      case _SkinAccess.crystals:
        return user.ownsSkin(skin.id) || _rewardOwnedSkinIds.contains(skin.id);
  ...
  Future<void> _loadSelectedSkin() async {
    try {
      ...
      if (selectedSkinId.isEmpty) {
        final savedSkin = await StorageService.loadSelectedBikeSkin();

        selectedSkinId = savedSkin.trim();
      }

      final normalized =
          _skins.any((skin) => skin.id == selectedSkinId && _isSkinUsable(skin))
          ? selectedSkinId
          : 'standard';
      ...
      if (widget.bike.effectiveActiveSkin.trim().isEmpty) {
        try {
          ...
          debugPrint('BIKE CUSTOMIZE INITIAL SKIN SYNC ERROR: $error');
          debugPrint('$stackTrace');
        }
      }
    } catch (error, stackTrace) {
      debugPrint('BIKE CUSTOMIZE LOAD SKIN ERROR: $error');
      debugPrint('$stackTrace');
  ...
  Future<void> _startCustomizeNativeShowroom() async {
    ...
    try {
      ...
      debugPrint('BIKE CUSTOMIZE NATIVE SHOWROOM START ERROR: $error');
      debugPrint('$stackTrace');
    }
  }
  ...
  Future<void> _applySkinMaterialToFrame(
    ...
    final materialInstanceName = skin.materialInstanceForFrame(targetFrame.id);
    ...
    if (skin.id == 'standard') {
      debugPrint(
        'BIKE CUSTOMIZE STANDARD MATERIAL SEND: '
        '${targetFrame.entityName} -> $materialInstanceName',
      );
      ...
      debugPrint(
        ...
        '${targetFrame.id} -> $materialInstanceName',
      );
      return;
    }
  ...
  Future<void> _applySkinVisual(
    ...
    try {
      ...
      // Start native showroom independently of the skin/material system.
      // On iOS the GLB material catalog may fail, but that must not prevent
      // the showroom camera motion from starting.
      await _startCustomizeNativeShowroom();

      if (!mounted) return;

      await _applySkinMaterialToFrame(skin, targetFrame);
    } catch (error, stackTrace) {
      debugPrint(
  ...
  void _scheduleMasterFrameApply() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      unawaited(_initializeMasterPreviewWhenReady());
    });
  }
  ...
  Future<void> _initializeMasterPreviewWhenReady() async {
    try {
      ...
      await _applySkinVisual(activeSkin, preserveFrame: true);
  ...
  Future<bool> _confirmFramePurchase(_FrameVariantOption frame) async {
  ...
  Future<bool> _unlockFrameWithCrystals(_FrameVariantOption frame) async {
    final ownerId = widget.bike.ownerId.trim();
    ...
    final result = await FirestoreSkinEntitlementService.instance
    ...
    switch (result.status) {
      ...
      case FrameUnlockStatus.alreadyOwned:
        ...
        setState(() {
          _ownedFrameIds = <String>{..._ownedFrameIds, frame.id};
  ...
  Future<void> _selectFrameVariant(String frameId) async {
    ...
    if (!_isFrameOwned(frame)) {
      await _applyFrame(frameId, persist: false);
      return;
    }

    // Owned frames can be activated immediately.
    await _applyFrame(frameId, persist: true);
  }
  ...
  Future<void> _unlockSelectedFrame(String frameId) async {
    ...
    if (_isFrameOwned(frame)) {
      ...
      await _applyFrame(frame.id, persist: true);
      return;
    }
  ...
  Future<void> _applyFrame(
    ...
    try {
      ...
      final activeSkin = _skinById(_selectedSkinId) ?? _skinById('standard')!;
  ...
  Future<bool> _showSkinAccessSheet(_SkinOption skin) async {
    if (!mounted) return false;
  ...
  Future<void> _unlockCrystalSkin(_SkinOption skin) async {
    final ownerId = widget.bike.ownerId.trim();
    final price = skin.crystalPrice ?? 0;
    ...
    switch (result.status) {
      ...
      case SkinUnlockStatus.alreadyOwned:
        ...
        try {
          ...
          debugPrint('BIKE CUSTOMIZE UNLOCKED SKIN ACTIVE SYNC ERROR: $error');
          debugPrint('$stackTrace');
  ...
  Future<bool> _confirmSkinPurchase(_SkinOption skin) async {
    final price = skin.crystalPrice ?? 0;
    final balanceAfter = _crystalBalance - price;
                ...
                Row(
                  children: [
                    Container(
                      ...
                      height: 46,
                      decoration: BoxDecoration(
                        color: skin.previewColor.withValues(alpha: 0.16),
                        borderRadius: BorderRadius.circular(15),
                        border: Border.all(
                          color: skin.previewColor.withValues(alpha: 0.30),
                        ),
                      ),
  ...
  Future<void> _selectSkin(String skinId) async {
    ...
    try {
      await _applySkinVisual(skin, preserveFrame: true);
      ...
      if (!canPersist) {
        ...
        final materialName = skin.materialInstanceForFrame(targetFrame.id);
      ...
      debugPrint('BIKE CUSTOMIZE SKIN ACTIVE AND SYNCED: ${skin.id}');
    } catch (error, stackTrace) {
      ...
      if (previousSkin != null && _masterFrameReady) {
        try {
          await _applySkinVisual(previousSkin, preserveFrame: true);
        } catch (rollbackError, rollbackStackTrace) {
          debugPrint('BIKE CUSTOMIZE SKIN ROLLBACK ERROR: $rollbackError');
  ...
  String get _bikeDescription {
    final values = <String>[
      _bikeTypeLabel,
      if (widget.bike.brand.trim().isNotEmpty) widget.bike.brand.trim(),
      if (widget.bike.model.trim().isNotEmpty) widget.bike.model.trim(),
    ];

    return values.join(' · ');
  }

  bool get _supportsMtbCustomization =>
      widget.bike.type == FirestoreBikeType.mtb ||
      widget.bike.type == FirestoreBikeType.kids;

  String get _customizeModelPath =>
      BikeModelResolver.resolveFirestoreModelPath(widget.bike.type);

  List<_SkinOption> get _availableSkins =>
      _supportsMtbCustomization ? _skins : const <_SkinOption>[];

  List<_FrameVariantOption> get _availableFrames => _supportsMtbCustomization
      ? _frameVariants
  ...
  Widget build(BuildContext context) {
    ...
    final busy = _savingSkin || _loadingEntitlements || _switchingFrame;
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
                    ...
                    children: [
                      _MobileLegendsCustomizeStage(
                        ...
                        preview: _BikePreviewCard(
                          ...
                          skin: selectedSkin,
                          modelPath: _customizeModelPath,
                          supportsMtbCustomization: _supportsMtbCustomization,
                          loading: _loadingSkin,
                          frameController: _frameController,
                          frames: _availableFrames,
          ...
          if (_celebrationSkinName != null && _celebrationAccentColor != null)
            Positioned.fill(
...
enum _CustomizeCategory { skins, frame }

enum _SkinAccess { owned, crystals, pro, locked }
...
class _BikePreviewCard extends StatelessWidget {
  const _BikePreviewCard({
    ...
    required this.skin,
    required this.modelPath,
    required this.supportsMtbCustomization,
  ...
  final String modelPath;
  final bool supportsMtbCustomization;
  final bool loading;
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          gradient: const RadialGradient(
            ...
            radius: 0.95,
            colors: [Color(0xFF0B281F), Color(0xFF04110D), Color(0xFF010504)],
          ),
          borderRadius: BorderRadius.circular(30),
        ),
        child: loading
            ...
            ? Stack(
                ...
                children: [
                  ...
                  Interactive3d(
                    key: const ValueKey<String>('munja-native-master-bike'),
                    ...
                    child: IgnorePointer(
                      child: ValueListenableBuilder<double>(
                        ...
                        builder: (context, stageAngle, child) {
                          return _ShowroomStageRing(rotationAngle: stageAngle);
                        },
                      ),
                    ),
                  ),
                  Positioned(
                    ...
                    child: IgnorePointer(
                      child: Container(
                        ...
                        child: Text(
                          AppText.t(frames[safeIndex].title).toUpperCase(),
                          style: const TextStyle(
                            color: MunjaColors.mint,
                            fontSize: 9,
                            fontWeight: FontWeight.w900,
            ...
            : Munja3DBikeViewer(
                key: ValueKey<String>(modelPath),
                modelPath: modelPath,
                applyDigitalTwinCustomization: false,
                height: 420,
                isLive: false,
...
class _ShowroomStageRing extends StatelessWidget {
  const _ShowroomStageRing({required this.rotationAngle});
  ...
  Widget build(BuildContext context) {
    return Align(
      ...
      child: SizedBox(
        ...
        height: 132,
        child: CustomPaint(
          painter: _ShowroomStagePainter(rotationAngle: rotationAngle),
        ),
      ),
    );
  }
...
class _ShowroomStagePainter extends CustomPainter {
  const _ShowroomStagePainter({required this.rotationAngle});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final normalizedRotation = (rotationAngle / maxShowroomAngle).clamp(
      -1.0,
      1.0,
    );
    ...
    canvas.drawOval(outerRect, glowPaint);
    ...
    canvas.drawOval(middleRect, middlePaint);
    ...
    canvas.drawArc(outerRect, 0.30, math.pi * 0.74, false, frontHighlightPaint);
  }
  ...
  void _drawOrbitMarker({
    ...
    final glow = Paint()
      ..color = MunjaColors.mint.withValues(alpha: alpha * 0.24)
      ..maskFilter = const MaskFilter.blur(BlurStyle.normal, 8);

    canvas.drawCircle(point, radius * 2.2, glow);

    final marker = Paint()..color = MunjaColors.mint.withValues(alpha: alpha);

    canvas.drawCircle(point, radius, marker);
  }

  @override
  bool shouldRepaint(covariant _ShowroomStagePainter oldDelegate) {
...
class _SkinSectionState extends State<_SkinSection> {
  ...
  int _indexForSkin(String skinId) {
    final index = _visibleSkins.indexWhere((skin) => skin.id == skinId);
  ...
  void _goRelative(int delta) {
    ...
    final next = (_pageIndex + delta).clamp(0, skins.length - 1);
  ...
  Widget build(BuildContext context) {
    ...
    if (skins.isEmpty) {
      return _CustomizeEmptyState(
        icon: Icons.palette_outlined,
        title: AppText.t('noSkinsAvailableYet'),
        subtitle: AppText.t('skinsAvailableLater'),
      );
    }
    ...
    if (widget.unlockingSkinId == selectedSkin.id) {
      ...
      status = selectedSkin.id == 'standard' ? 'OWNED' : 'EQUIPPED';
    ...
    return SizedBox(
      ...
      child: Column(
        children: [
                    ...
                    children: [
                      Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          boxShadow: [
                            BoxShadow(
                              color: selectedSkin.previewColor.withValues(
                                alpha: 0.40,
                              ),
                              blurRadius: 10,
                            ),
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            Text(
                              AppText.t(selectedSkin.title).toUpperCase(),
                              maxLines: 1,
                              overflow: TextOverflow.ellipsis,
                              style: const TextStyle(
                                color: Colors.white,
              ...
              children: [
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return AnimatedPadding(
                      ...
                      child: _SkinCarouselCard(
                        skin: skin,
                        selected: skin.id == widget.selectedSkinId,
                        focused: focused,
                        usable: widget.canUseSkin(skin),
                        unlocking: widget.unlockingSkinId == skin.id,
                        disabled: widget.loading || widget.saving,
                        onTap: () {
                          _controller.animateToPage(
                            index,
                ...
                Positioned(
                  ...
                  child: _MiniCarouselArrow(
                    icon: Icons.chevron_left_rounded,
                    disabled:
                        _pageIndex <= 0 || widget.loading || widget.saving,
                    onTap: () => _goRelative(-1),
                  ),
                ),
          ...
          _CarouselDots(count: skins.length, activeIndex: _pageIndex),
          const SizedBox(height: 4),
        ],
      ),
    );
            ...
            gradient: LinearGradient(
              colors: disabled
                  ...
                  : [
                      ...
                      const Color(0xFF70D8FF).withValues(alpha: 0.09),
                    ],
            ),
            borderRadius: BorderRadius.circular(999),
            border: Border.all(
...
class _SkinAccessBadge extends StatelessWidget {
  const _SkinAccessBadge({required this.label, required this.icon});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          Icon(icon, size: 10, color: const Color(0xFFC981FF)),
        ...
        child: Material(
          ...
          child: InkWell(
            ...
            child: Container(
              ...
              child: Icon(icon, color: MunjaColors.mint, size: 17),
            ),
          ),
        ),
      ),
        ...
        return AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: selected
                ? [
                    BoxShadow(
                      color: MunjaColors.mint.withValues(alpha: 0.30),
                      blurRadius: 7,
                    ),
                  ]
                : null,
...
class _SkinPreviewArtwork extends StatelessWidget {
  const _SkinPreviewArtwork({required this.skin, required this.focused});
            ...
            child: DecoratedBox(
              decoration: BoxDecoration(
                gradient: RadialGradient(
                  ...
                  radius: 0.88,
                  colors: [
                    Colors.white.withValues(alpha: focused ? 0.035 : 0.015),
                    const Color(
                      0xFF06110E,
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              ...
              gradient: LinearGradient(
                ...
                colors: [
                  Colors.white.withValues(alpha: focused ? 0.045 : 0.018),
                  const Color(0xFF07130F).withValues(alpha: 0.94),
                                ...
                                child: Text(
                                  ...
                                  style: TextStyle(
                                    color: usable
                                        ? MunjaColors.mint
                                        : isPro || isLocked
                                        ? const Color(0xFFC981FF)
                                        : const Color(0xFF70D8FF),
                                    fontSize: focused ? 6.5 : 5.4,
                                    fontWeight: FontWeight.w900,
                                  ),
                                ),
                              ),
                  ...
                  Positioned(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        boxShadow: [
                          BoxShadow(
                            color: MunjaColors.mint.withValues(alpha: 0.38),
                            blurRadius: 9,
                          ),
                        ],
                      ),
...
class _CustomizeEmptyState extends StatelessWidget {
  const _CustomizeEmptyState({
    required this.icon,
    required this.title,
    required this.subtitle,
  });
  ...
  Widget build(BuildContext context) {
    if (frames.isEmpty) {
      return _CustomizeEmptyState(
        ...
        title: AppText.t('noFramesAvailableYet'),
        subtitle: AppText.t('framesAvailableLater'),
      );
    }
    ...
    final selectedCanAfford =
        selectedFrame.included || crystalBalance >= selectedFrame.crystalPrice;
    final selectedUnlocking = unlockingFrameId == selectedFrame.id;
          ...
          SizedBox(
            ...
            child: ListView.separated(
              ...
              itemBuilder: (context, index) {
                ...
                final canAfford =
                    frame.included || crystalBalance >= frame.crystalPrice;
                  ...
                  child: InkWell(
                    ...
                    child: AnimatedContainer(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(
                          color: selected
                              ? MunjaColors.mint.withValues(alpha: 0.78)
                              : Colors.white.withValues(alpha: 0.06),
                        ...
                        children: [
                          Row(
                            ...
                            children: [
                              ...
                              Flexible(
                                child: Text(
                                  AppText.t(frame.title).toUpperCase(),
                                  maxLines: 1,
                                  overflow: TextOverflow.ellipsis,
                                  style: TextStyle(
                                    color: selected
              ...
              child: FilledButton(
                ...
                child: selectedUnlocking
                    ...
                    : Row(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          const Icon(Icons.diamond_rounded, size: 15),
                          const SizedBox(width: 7),
                          Text(
                            selectedCanAfford
      ...
      child: Row(
        children: [
          Container(
            ...
            child: Icon(icon, color: MunjaColors.mint, size: 24),
          ),
